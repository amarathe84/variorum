...
void print_all_sensors(void *buf)
{
    ...
    printf("RRR name | offset | scale_factor | units\n");
	...
	for (i = 0; i < be16toh(hb->nr_sensors); i++) {
        ...
        printf("RRR %s | 0x%lx | %d | %s\n", md[i].name, offset, scale, md[i].units);
                
		if (md[i].structure_type == OCC_SENSOR_READING_FULL)
        {
            //printf("RRR Reading sensor\n");
			sample = read_sensor(hb, offset, SENSOR_SAMPLE);
        }
        /* read_counter not used by the counters we want */
		else
        {
            //printf("RRR Reading counter\n");
		...
		if (be16toh(md[i].type) == OCC_SENSOR_TYPE_POWER) {
            ...
            //printf("RRR hb=%ld, offset=%ld, scale=%ld\nRRR freq=%ld, scale_factor=%ld\n",
            //       hb->names_offset, offset, scale, freq, be32toh(md[i].scale_factor));
            //printf("RRR FP(freq)=%lu, FP(scale)=%lu, energy=%lu\n",
            //       (uint64_t)TO_FP(freq), (uint64_t)TO_FP(scale), energy);
            //printf("RRR conv_energy=%lu\n", 
			//       (uint64_t)(energy / TO_FP(freq)));
		} else {
			printf("%5d %-20s %16lu %s\n", i, md[i].name,
			(uint64_t)(sample * TO_FP(scale)), md[i].units);
            //printf("RRR hb=%ld, offset=%ld\n",
            //       hb->names_offset, offset);
		}
	}
}
